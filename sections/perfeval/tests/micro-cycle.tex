...
(2) and (3b). It shows the sum of instructions executed over 50 requests for
both isolation designs, separately for RSA and ECDHE ciphers.  The SGX
instructions reported exclude the the instructions executed at enclave
start-up.
...
and 22\% for the same design but for ECDHE ciphers. Note that, there could be a
performance impact on the key-inside compared to the key-outside
implementations, because of the effect of memory encryption in the enclave, as
discussed in Section~\ref{sec:mem-analysis}.
